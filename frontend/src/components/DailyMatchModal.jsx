...
function DailyMatchModal({ character, onClose }) {
  ...
  return (
    ...
    >
      <div
        className={`relative bg-gradient-to-br from-purple-500 via-pink-500 to-rose-500 rounded-3xl shadow-2xl p-8 max-w-md w-full mx-4 transform transition-all duration-500 ${
      ...
      >
        ...
        <p className="text-center text-purple-100 mb-6">
          You've been matched with {character.name || character.cardData?.data?.name}!
        </p>
        ...
        <div className="flex justify-center mb-6">
          <div className="w-48 h-48 rounded-full overflow-hidden border-4 border-white shadow-xl ring-4 ring-yellow-300 ring-opacity-50">
            <img
              ...
              alt={character.name || character.cardData?.data?.name}
        ...
        <p className="text-center text-white text-lg mb-6 font-medium">
          We picked someone special from your library just for you today! 🎁
        </p>

        {/* Close Button */}
        <button
          onClick={onClose}
          className="w-full bg-white text-purple-600 font-bold py-3 px-6 rounded-xl hover:bg-purple-50 transition-colors shadow-lg"
        >
          Start Chatting
        </button>
      </div>
