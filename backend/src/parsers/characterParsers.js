  ...
  for (let i = 0; i < sections.length; i += 2) {
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        // If this line doesn't match the time block format, it's a continuation of the previous block's activity
        if (blocks.length > 0) {
          ...
          // First line doesn't match time block format - just skip it (might be extra whitespace or day header continuation)
          console.log(`⚠️  Skipping line (no active block): "${trimmedLine}"`);
        }
      }
    }
